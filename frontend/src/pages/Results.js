...
const Results = ({ results, userProfile }) => {
  ...
  // Calculate total from citations
  const calculateTotal = () => {
    if (!results.citations) return "$0.00";
    const total = results.citations.reduce((sum, citation) => {
      const amount = parseFloat(citation.fine.replace(/[$,]/g, ''));
      return sum + amount;
    }, 0);
        ...
        <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8 flex-1 w-full">
          ...
          <div className="bg-white border border-[#dfe1e2] rounded-sm p-4 sm:p-6 shadow-sm text-center py-12 sm:py-16">
            ...
            <p className="text-[#71767a] max-w-md mx-auto mb-6 sm:mb-8 text-sm sm:text-base px-4">
              {results.message || "No records found for the provided citation number. Please verify and try again."}
            </p>
            <Button
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-6 sm:py-8 flex-1 w-full">
        {/* STATUS Header */}
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-[#1b1b1b] mb-2">
            STATUS
          </h1>
        ...
        <div className="bg-[#f0f0f0] border border-[#dfe1e2] rounded-sm p-4 sm:p-6 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-base sm:text-lg text-[#1b1b1b] font-semibold">{userProfile?.name || results.name}</p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-[#71767a] mb-1">Date of Birth</p>
              <p className="text-base sm:text-lg font-mono text-[#1b1b1b]">{userProfile?.dob || "N/A"}</p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-[#71767a] mb-1">Address</p>
              <p className="text-sm sm:text-base text-[#1b1b1b]">{userProfile?.address || "N/A"}</p>
            </div>
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-[#71767a] mb-1">Phone</p>
              <p className="text-sm sm:text-base font-mono text-[#1b1b1b]">{userProfile?.phone || "N/A"}</p>
            </div>
          </div>
        </div>

        {/* Citations Table - Mobile Optimized */}
        ...
        <div className="bg-[#1a4480] text-white rounded-sm p-4 sm:p-6 mb-6">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xl sm:text-2xl font-bold font-mono">{calculateTotal()}</span>
          </div>
        </div>
